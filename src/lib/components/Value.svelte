...
<Card bind:input_value={value} bind:card_input={input} disabled={true}>
    <h2 slot="header" class="text-4xl md:text-5xl dark:text-white text-navy">Value</h2>
    <p slot="unit" class="text-4xl dark:text-white text-navy inline-block">{$symbol}</p>
</Card>